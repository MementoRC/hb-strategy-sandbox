...
class TestPerformanceBenchmarks:
    ...
    async def test_balance_operations_speed(self):
        ...
        for i in range(num_operations):
            ...
            available = sandbox.balance.get_available_balance("USDT")
        ...
        assert operations_per_second > 1000, f"Balance operations too slow: {operations_per_second} ops/sec"
    ...
    async def test_event_system_performance(self):
        ...
        for i in range(num_events):
            sandbox.event.emit_event(MarketEvent.ORDER_CREATED, {
                "order_id": f"test-{i}",
                "timestamp": time.time(),
            })
